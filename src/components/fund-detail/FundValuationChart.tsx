import React, { useEffect, useState } from 'react';
import ReactEChartsCore from 'echarts-for-react/lib/core';
import * as echarts from 'echarts/core';
import { LineChart } from 'echarts/charts';
import { GridComponent, TooltipComponent, MarkLineComponent } from 'echarts/components';
import { CanvasRenderer } from 'echarts/renderers';
import { fetchFundValuationData } from '@/services/fundDetailApi';
import { useSettingsStore } from '@/stores/settingsStore';
import { Loader2 } from 'lucide-react';

echarts.use([LineChart, GridComponent, TooltipComponent, MarkLineComponent, CanvasRenderer]);

// 交易时间
const TIME_DATA = [
  "09:30", "09:31", "09:32", "09:33", "09:34", "09:35", "09:36", "09:37", "09:38", "09:39",
  "09:40", "09:41", "09:42", "09:43", "09:44", "09:45", "09:46", "09:47", "09:48", "09:49",
  "09:50", "09:51", "09:52", "09:53", "09:54", "09:55", "09:56", "09:57", "09:58", "09:59",
  "10:00", "10:01", "10:02", "10:03", "10:04", "10:05", "10:06", "10:07", "10:08", "10:09",
  "10:10", "10:11", "10:12", "10:13", "10:14", "10:15", "10:16", "10:17", "10:18", "10:19",
  "10:20", "10:21", "10:22", "10:23", "10:24", "10:25", "10:26", "10:27", "10:28", "10:29",
  "10:30", "10:31", "10:32", "10:33", "10:34", "10:35", "10:36", "10:37", "10:38", "10:39",
  "10:40", "10:41", "10:42", "10:43", "10:44", "10:45", "10:46", "10:47", "10:48", "10:49",
  "10:50", "10:51", "10:52", "10:53", "10:54", "10:55", "10:56", "10:57", "10:58", "10:59",
  "11:00", "11:01", "11:02", "11:03", "11:04", "11:05", "11:06", "11:07", "11:08", "11:09",
  "11:10", "11:11", "11:12", "11:13", "11:14", "11:15", "11:16", "11:17", "11:18", "11:19",
  "11:20", "11:21", "11:22", "11:23", "11:24", "11:25", "11:26", "11:27", "11:28", "11:29", "11:30",
  "13:00", "13:01", "13:02", "13:03", "13:04", "13:05", "13:06", "13:07", "13:08", "13:09",
  "13:10", "13:11", "13:12", "13:13", "13:14", "13:15", "13:16", "13:17", "13:18", "13:19",
  "13:20", "13:21", "13:22", "13:23", "13:24", "13:25", "13:26", "13:27", "13:28", "13:29",
  "13:30", "13:31", "13:32", "13:33", "13:34", "13:35", "13:36", "13:37", "13:38", "13:39",
  "13:40", "13:41", "13:42", "13:43", "13:44", "13:45", "13:46", "13:47", "13:48", "13:49",
  "13:50", "13:51", "13:52", "13:53", "13:54", "13:55", "13:56", "13:57", "13:58", "13:59",
  "14:00", "14:01", "14:02", "14:03", "14:04", "14:05", "14:06", "14:07", "14:08", "14:09",
  "14:10", "14:11", "14:12", "14:13", "14:14", "14:15", "14:16", "14:17", "14:18", "14:19",
  "14:20", "14:21", "14:22", "14:23", "14:24", "14:25", "14:26", "14:27", "14:28", "14:29",
  "14:30", "14:31", "14:32", "14:33", "14:34", "14:35", "14:36", "14:37", "14:38", "14:39",
  "14:40", "14:41", "14:42", "14:43", "14:44", "14:45", "14:46", "14:47", "14:48", "14:49",
  "14:50", "14:51", "14:52", "14:53", "14:54", "14:55", "14:56", "14:57", "14:58", "14:59", "15:00",
];

interface FundValuationChartProps {
  fundCode: string;
}

export default function FundValuationChart({ fundCode }: FundValuationChartProps) {
  const { darkMode } = useSettingsStore();
  const [isLoading, setIsLoading] = useState(true);
  const [chartData, setChartData] = useState<number[]>([]);
  const [dwjz, setDwjz] = useState(0);

  useEffect(() => {
    const loadData = async () => {
      setIsLoading(true);
      try {
        const result = await fetchFundValuationData(fundCode);
        setChartData(result.dataList.map(d => d.value));
        setDwjz(result.DWJZ);
      } catch (error) {
        console.error('Failed to load fund valuation data:', error);
      } finally {
        setIsLoading(false);
      }
    };

    loadData();
  }, [fundCode]);

  // 计算Y轴范围
  const getYAxisRange = () => {
    if (chartData.length === 0) return { min: -1, max: 1, interval: 0.5 };
    const absMax = Math.max(
      Math.abs(Math.max(...chartData)),
      Math.abs(Math.min(...chartData))
    );
    const range = Math.max(absMax, 0.5);
    return { min: -range, max: range, interval: range / 4 };
  };

  const yAxisRange = getYAxisRange();
  const axisLabelColor = darkMode ? 'rgba(255,255,255,0.6)' : '#999';
  const splitLineColor = darkMode ? 'rgba(255,255,255,0.1)' : '#eee';

  const getChartOption = () => ({
    backgroundColor: 'transparent',
    tooltip: {
      trigger: 'axis',
      backgroundColor: darkMode ? 'rgba(30, 30, 30, 0.9)' : 'rgba(255, 255, 255, 0.95)',
      borderColor: darkMode ? 'rgba(255,255,255,0.1)' : '#eee',
      textStyle: { color: darkMode ? '#fff' : '#333' },
      formatter: (p: any) => {
        const val = p[0]?.value;
        if (val === undefined) return '';
        const estimatedNav = (dwjz * (1 + 0.01 * val)).toFixed(4);
        return `时间：${p[0].name}<br />估算涨跌幅：${val.toFixed(2)}%<br />估算净值：${estimatedNav}元`;
      },
    },
    grid: {
      top: 30,
      bottom: 30,
      left: 50,
      right: 60,
    },
    xAxis: {
      type: 'category',
      data: TIME_DATA,
      axisLabel: {
        color: axisLabelColor,
        interval: (_: number, value: string) => {
          return ['09:30', '10:30', '13:00', '14:00', '15:00'].includes(value);
        },
        formatter: (value: string) => value === '13:00' ? '11:30/13:00' : value,
      },
      axisLine: {
        onZero: false,
        lineStyle: { color: splitLineColor },
      },
    },
    yAxis: [
      {
        type: 'value',
        min: yAxisRange.min,
        max: yAxisRange.max,
        interval: yAxisRange.interval,
        axisLabel: {
          color: (val: number) => val > 0 ? '#f56c6c' : val < 0 ? '#4eb61b' : axisLabelColor,
          formatter: (val: number) => val.toFixed(2) + '%',
        },
        splitLine: {
          lineStyle: { type: 'dashed', color: splitLineColor },
        },
      },
      {
        type: 'value',
        min: yAxisRange.min,
        max: yAxisRange.max,
        interval: yAxisRange.interval,
        axisLabel: {
          color: (val: number) => val > 0 ? '#f56c6c' : val < 0 ? '#4eb61b' : axisLabelColor,
          formatter: (val: number) => (dwjz * (1 + 0.01 * val)).toFixed(4),
        },
        splitLine: { show: false },
      },
    ],
    series: [
      {
        name: '估算涨跌幅',
        type: 'line',
        data: chartData.map(v => v.toFixed(2)),
        smooth: false,
        symbol: 'none',
        lineStyle: { width: 2, color: '#409eff' },
        areaStyle: {
          color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
            { offset: 0, color: 'rgba(64, 158, 255, 0.3)' },
            { offset: 1, color: 'rgba(64, 158, 255, 0.05)' },
          ]),
        },
        markLine: {
          silent: true,
          symbol: 'none',
          animation: false,
          label: { show: false },
          lineStyle: { type: 'solid', color: axisLabelColor },
          data: [{ yAxis: 0 }],
        },
      },
    ],
  });

  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-[260px]">
        <Loader2 className="h-8 w-8 animate-spin text-primary" />
      </div>
    );
  }

  return (
    <div className="h-[260px]">
      <ReactEChartsCore
        echarts={echarts}
        option={getChartOption()}
        style={{ height: '100%', width: '100%' }}
        opts={{ renderer: 'canvas' }}
      />
    </div>
  );
}
