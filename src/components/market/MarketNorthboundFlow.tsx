...
import { fetchNorthboundFlowData, type CrossBorderFlowData } from '@/services/marketApi';
...
export default function MarketNorthboundFlow() {
  ...
  useEffect(() => {
    const loadData = async () => {
      setIsLoading(true);
      try {
        const result = await fetchNorthboundFlowData();
        setData(result);
      } catch (error) {
        console.error('Failed to load northbound flow data:', error);
      } finally {
        setIsLoading(false);
      }
    };
