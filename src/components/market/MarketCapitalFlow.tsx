...
import { fetchCapitalFlowData, fetchTurnoverData, type CapitalFlowData, type TurnoverData } from '@/services/marketApi';
...
export default function MarketCapitalFlow() {
  ...
  useEffect(() => {
    const loadData = async () => {
      setIsLoading(true);
      try {
        const [flow, turnover] = await Promise.all([
  ...
  // 计算成交额和涨跌数
  const totalTurnover = turnoverData.length >= 2
    ? ((turnoverData[0].f6 + turnoverData[1].f6) / 100000000).toFixed(2)
    : '0.00';
  const upCount = turnoverData.length >= 2
    ? turnoverData[0].f104 + turnoverData[1].f104
    : 0;
  const downCount = turnoverData.length >= 2
    ? turnoverData[0].f105 + turnoverData[1].f105
    : 0;
  const flatCount = turnoverData.length >= 2
    ? turnoverData[0].f106 + turnoverData[1].f106
    : 0;
